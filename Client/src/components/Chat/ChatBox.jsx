...
const ChatBox = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            if (showTriggerSmileyForMsgId !== null) {
                const triggerSmileyElement = document.getElementById(`smiley-trigger-${showTriggerSmileyForMsgId}`);
                if (triggerSmileyElement && triggerSmileyElement.contains(event.target)) {
            ...
            if (showQuickReactionBarForMsgId !== null) {
                const quickReactionBarElement = document.getElementById(`reaction-menu-${showQuickReactionBarForMsgId}`);
                if (quickReactionBarElement && quickReactionBarElement.contains(event.target)) {
                    clickedInsideAControlledElement = true;
                }
            }
            ...
            if (fullPickerMessageId !== null) {
                const fullPickerElement = document.getElementById(`full-picker-${fullPickerMessageId}`);
            ...
            if (selectedMsgIdForOptions !== null) {
                const optionsDropdownElement = document.getElementById(`options-menu-${selectedMsgIdForOptions}`);
                const optionsButton = document.getElementById(`options-button-${selectedMsgIdForOptions}`);
                if ((optionsDropdownElement && optionsDropdownElement.contains(event.target)) ||
    ...
    }, [showTriggerSmileyForMsgId, showQuickReactionBarForMsgId, fullPickerMessageId, selectedMsgIdForOptions]);
    ...
    const handleOptionClick = (option, msg) => {
        if (option === 'copy') {
            ...
            setPinnedMsgs((prev) => [msg, ...prev.filter((pinnedMsg) => pinnedMsg._id !== msg._id)]);
        } else if (option === 'star') {
            setStarredMsgs((prev) => {
    ...
    const getReactionCounts = (reactions) => {
        if (!reactions) return {};
        const counts = {};
        if (Array.isArray(reactions)) { // Ensure reactions is an array
            reactions.forEach(r => {
    ...
    return (
        <div className="h-screen flex flex-col bg-[url('/bg-chat-dark.png')] bg-cover">
                ...
                {sortedMessages.map((m) => {
                    ...
                    return (
                        <div
                            ...
                            className={`relative group max-w-[100%] flex ${isMine ? 'justify-end' : 'justify-start'}`} // This parent aligns the whole message block (including bubble and icons)
                            onMouseEnter={() => {
                                // Only show smiley trigger if no other reaction/picker is active for this message
                                if (showQuickReactionBarForMsgId !== m._id && fullPickerMessageId !== m._id) {
                        ...
                        >
                            ...
                            <div className={`relative px-4 py-2 rounded-xl text-sm break-words transition-all duration-200
                                ...
                                {showTriggerSmileyForMsgId === m._id && !showQuickReactionBarForMsgId && !fullPickerMessageId && (
                                    <div
                                        ...
                                        className={`absolute z-20 p-1 rounded-full bg-gray-700 shadow-lg text-white text-lg cursor-pointer
                                                ${isMine ? 'left-[-35px]' : 'right-[-35px]'} top-1/2 -translate-y-1/2`}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                ...
                                {showQuickReactionBarForMsgId === m._id && (
                                    <div
                                        ...
                                        className={`absolute z-20 flex gap-1 bg-gray-700 p-1 rounded-full shadow-lg transition-all duration-100 ease-in-out
                                                ${isMine ? 'left-[-270px]' : 'right-[-270px]'} top-1/2 -translate-y-1/2`}
                                    >
                                ...
                                <div className="text-xs text-gray-900 text-[14px] font-[600] mb-1">{m.user?.name || 'Unknown'}</div>
                                ...
                                <div>
                                    {m.text.startsWith('Shared file:') ? (
                                        <a href={m.text.replace('Shared file: ', '')} target="_blank" rel="noopener noreferrer" className="underline text-white">
                                ...
                                <div className="text-[10px] text-gray-300 text-right mt-1 flex flex-wrap justify-end gap-1 items-center">
                                    {pinnedMsgs.some(p => p._id === m._id) && <FaThumbtack className="text-yellow-400" />}
                                    {starredMsgs.has(m._id) && <FaStar className="text-yellow-300" />}
                                ...
                                <div
                                    ...
                                    className={`absolute top-1 ${isMine ? 'right-1' : 'right-1'} cursor-pointer hidden group-hover:block`}
                                    onClick={() => setSelectedMsgIdForOptions(m._id === selectedMsgIdForOptions ? null : m._id)}>
                                    <span className="text-xs text-gray-900"><FaAngleDown /></span>
                                </div>
                                ...
                                {selectedMsgIdForOptions === m._id && (
                                    <div
                                        ...
                                        className={`absolute z-20 bg-gray-800 text-white font-[500] border rounded shadow-md text-sm top-6 ${isMine ? 'left-[-75px]' : 'right-[-68px]'}`}>
                                        {options.map((opt) => (
                                            <div key={opt} className="px-4 py-2 hover:bg-gray-950 cursor-pointer transition" onClick={() => handleOptionClick(opt, m)}>
                            ...
                            {fullPickerMessageId === m._id && (
                                <div
                                    ...
                                    className={`absolute z-30 ${isMine ? 'left-0' : 'right-0'} bottom-10`}
                                >
                                    <Picker
                                        onEmojiClick={(emojiData) => handleReaction(m._id, emojiData.emoji)}
                                        height={350}
                                        width={280}
                                        emojiVersion="1.0"
                                    />
            ...
            <div className="sticky bottom-0 left-0 w-full border-t px-4 py-3 bg-[#c4c7c9] z-10">
                    ...
                    <div className="relative">
                        ...
                        {showAttachmentMenu && (
                            <div className="absolute bottom-full mb-2 left-1 z-30 bg-white border shadow-lg rounded-lg p-2 w-52">
                                {[{ type: 'document', label: '📄 Document' }, { type: 'image', label: '🖼️ Photo' }, { type: 'audio', label: '🎧 Audio' }, { type: 'video', label: '🎥 Video' }].map((item) => (
                                    <div key={item.type} className="cursor-pointer p-2 hover:bg-gray-100 rounded transition" onClick={() => handleAttachClick(item.type)}>
                                        {item.label}
                                    </div>
                                ))}
                            </div>
                ...
                {showEmojiPicker && (
                    <div className="mt-2">
                        <Picker onEmojiClick={onEmojiClick} emojiVersion="1.0" height={350} width="100%" />
                    </div>
                )}
            </div>
        </div>
