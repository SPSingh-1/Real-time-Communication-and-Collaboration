...
const CalendarView = () => {
  ...
  const handleDateClick = (date) => {
    ...
    if (selected < new Date(today.toDateString())) {
      toast.warn('⚠️ Cannot select past dates.');
      return;
    }
  ...
  const addOrUpdateEvent = async () => {
    ...
    console.log("✅ Outgoing event payload:", eventData);
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6 max-h-96 overflow-y-auto w-[300px] transition-transform hover:scale-105">
          {selectedDate && (
            <div className="mt-6 bg-gray-50 rounded-xl p-4 shadow-md animate-fadeInUp">
              <h4 className="text-lg font-bold mb-3 border-b-2 border-gray-200">Upcoming Events</h4>
      ...
      {showPrompt && selectedDate && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md transition-all scale-95 hover:scale-100">
            <h2 className="text-lg font-bold mb-4 text-center border-b pb-2">Manage Events for {selectedDate.toDateString()}</h2>
